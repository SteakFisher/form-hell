...
import MultipleChoiceGridProps
  from "@/interfaces/form-component-interfaces/multiple-choice-grid/MultipleChoiceGridProps";
...
export default function MultipleChoiceGridComponent({
  ...
  formResponses[id] = { type: "multiple-choice-grid", selected: selected } as MultipleChoiceGridResponse;
      ...
      <CardContent>
        <div className="flex min-h-8 items-center space-x-2">
          ...
          {props.columns.map((column, index) => (
            <span key={index + "-column"} className="flex-1 truncate text-center">
							{column.value}
						</span>
          ))}
        </div>
        {props.rows.map((row, index) => {
          return (
            <RadioGroup key={index + "-row"} onValueChange={(value) => {
              ...
              >
                ...
                {props.columns.map((column, index) => (
                  ...
                  >
                    {isRadio ? (
                      <RadioGroupItem className="size-5 shrink-0" value={column.id}/>
                    ) : (
                      <Checkbox
                        className="disabled:cursor-default disabled:opacity-100"
                        checked={selected[row.id] && selected[row.id].has(column.id)}
                        onCheckedChange={(checked) => {
                          if (checked) {
                            if (!selected[row.id]) {
                              selected[row.id] = new Set([column.id])
