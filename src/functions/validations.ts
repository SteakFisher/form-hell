			...
			.object(
				{
					...
					selected: z
						.union([
							...
							z.record(z.string(), z.set(z.string())),
						])
						.optional(),
...
function validateTextInput(
	...
	if (item.props.lengthType === "characters") {
		if (item.props.maxLength)
			inputResponseValidate = inputResponseValidate.max(
				item.props.maxLength,
				{
					message: `Input can't be longer than ${item.props.maxLength} characters!`,
				},
			);
		if (item.props.minLength)
			inputResponseValidate = inputResponseValidate.min(
				item.props.minLength,
				{
					message: `Input can't be shorter than ${item.props.minLength} characters!`,
				},
			);
	}

	if (item.props.regexPattern)
		inputResponseValidate = inputResponseValidate.regex(
			new RegExp(item.props.regexPattern, item.props.regexFlags),
			{
				message: `Input doesn't match the regex ${item.props.regexPattern}`,
			},
		);
	...
	if (item.props.lengthType === "words") {
		if (
			item.props.maxLength &&
			item.props.maxLength < response.input.split(" ").length
		) {
			errors[item.id] =
				`Input can't be longer than ${item.props.maxLength} words!`;
		}
		if (
			item.props.minLength &&
			item.props.minLength > response.input.split(" ").length
		) {
			errors[item.id] =
				`Input can't be longer than ${item.props.maxLength} words!`;
		}
...
function validateDropdownInput(
	item: FormItem,
	...
	const zodLiterals = item.props.items.map((option) =>
		z.literal(option.id),
	...
	let dropdownResponseValidate = z
		.object(
			{
		...
		.object(
			{
				range: z
					...
					.min(item.props.min)
					.max(item.props.max, {
						message: `Value must be between ${item.props.min} and ${item.props.max}`,
					}),
...
function validateMultipleChoiceInput(
	item: FormItem,
	...
	if (
		(!response.selected || Object.keys(response.selected).length == 0) &&
		!item.props.required
	)
		return;

	// Workaround for zod to work tsk tsk
	const zodLiteralsKeys = item.props.items.map((option) =>
		z.literal(option.id),
	) as unknown as readonly [
		z.ZodLiteral<string>,
		z.ZodLiteral<string>,
	...
	let multipleChoiceResponseValidate = z
		.object(
			{
				selected: z.record(
	...
	if (
		Object.keys(response.selected).length > 1 &&
		item.props.allowMultiple &&
		response.selected["other"] != undefined
	) {
...
function validateMultipleChoiceGridInput(
	item: FormItem,
	response: MultipleChoiceGridResponse,
	errors: Errors,
	finalResponse: FormResponses,
) {
	...
	// Workaround for zod to work tsk tsk
	const zodLiteralsRows = item.props.rows.map((option) =>
		z.literal(option.id),
	) as unknown as readonly [
		z.ZodLiteral<string>,
		z.ZodLiteral<string>,
	...
	const zodLiteralColumns = item.props.columns.map((option) =>
		z.literal(option.id),
	) as unknown as readonly [
		z.ZodLiteral<string>,
		z.ZodLiteral<string>,
	...
	gridProps.rows.map((row) => {
		if (!selected[row.id]) requireFlag = true;
			...
			selected[row.id].forEach((entry) => {
				if (!gridProps.columns.map((column) => column.id).includes(entry))
					validFlag = true;
			});

			if (selected[row.id].size > 1 && !gridProps.allowMultiple)
				tooManyEntriesFlag = true;
		}
	});

	if (requireFlag && item.props.required)
		errors[item.id] = "This field is required";
